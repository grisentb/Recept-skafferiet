...
Future<Response> getRecipesFromCategoryHandler(
    Request request, id, sessionToken, category) async {
  var session = await checkSession(id, sessionToken);
  if (session) {
...
Future<Response> getMyRecipesHandler(
    Request request, userId, sessionToken) async {
  var session = await checkSession(userId, sessionToken);
  var recipes = [];
  var relations = [];
  if (session) {
    await for (var streamedRelations
        in relationalCollection.find({'user_id': userId})) {
      relations.add(json.encode(streamedRelations));
    }
    for (var relation in relations) {
      print(json.decode(relation)['recipe_id']);
      var recipe = await recipeCollection
          .findOne({'url': json.decode(relation)['recipe_id']});
...
Future<Response> deleteRelationHandler(
    Request request, userId, sessionToken) async {
  var session = await checkSession(userId, sessionToken);
  if (session) {
    var codec = Utf8Codec();
    ...
    var res = await relationalCollection
...
Future<Response> deleteRecipeHandler(
    Request request, userId, sessionToken) async {
  var session = await checkSession(userId, sessionToken);
  if (session) {
    var codec = Utf8Codec();
    ...
    await relationalCollection
        .deleteOne({'user_id': userId, 'recipeID': recipeId});
